...
## Inputs
- A spec file path (e.g. `specs/PROJ-1234-2025-01-15-auth-flow.md`)
...
- If `cortex ask` returns "No DB found", fall back to reading `knowledge/` files directly — check `STANDARDS.md` and design-system files first, then recent specs for prior patterns. If cortex returns no results (DB exists, query matched nothing), proceed but flag ungrounded component references in task notes
